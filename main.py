"""Train and evaluate MMKGAT using the original experiment procedure."""
...
def evaluate(model, dataloader, Ks, device):
    """Compute recommendation scores and mean ranking metrics for test users."""
    ...
    user_ids_batches = [
        user_ids[i : i + test_batch_size] for i in range(0, len(user_ids), test_batch_size)
    ]
...
def train(args):
    """Alternate CF and KG optimization, then update graph attention each epoch."""
    # seed
    ...
    init_feats = getattr(data, "entity_init_features", None)
    ...
    model = KGAT(
        ...
        user_pre_embed,
        item_pre_embed,
        init_entity_features=init_feats,
        has_image_rel_id=args.has_image_rel_id,
        image_align_lambda=args.image_align_lambda,
    )
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        logging.info(
            "Update Attention: Epoch {:04d} | Total Time {:.1f}s".format(epoch, time() - time5)
        )

        logging.info(
            "CF + KG Training: Epoch {:04d} | Total Time {:.1f}s".format(epoch, time() - time0)
        )
        ...
        if (epoch % args.evaluate_every) == 0 or epoch == args.n_epoch:
            ...
            # Preserve model selection by recall at the smallest requested K.
