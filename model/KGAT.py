"""KGAT with gated external entity features and item-image alignment loss."""
...
class KGAT(nn.Module):
    ...
    def __init__(
        ...
        user_pre_embed=None,
        item_pre_embed=None,
        init_entity_features=None,
        has_image_rel_id=None,
        image_align_lambda=0.0,
    ):
        ...
        self._init_entity_features = None  # Plain attribute; not a registered buffer.
        self.feat_proj = None
        if init_entity_features is not None:
            self.register_buffer("init_entity_features", init_entity_features.float())

            d_clip = init_entity_features.size(1)
            if d_clip != args.embed_dim:
                self.feat_proj = nn.Linear(d_clip, args.embed_dim)
                nn.init.xavier_uniform_(self.feat_proj.weight)
            # Learn a gate for combining table embeddings and external features.
            self.alpha = nn.Parameter(
                torch.tensor(0.5)
            )  # Clamped to [0, 1] when combining embeddings.

        # Item-image alignment loss settings.
        self.has_image_rel_id = has_image_rel_id
        self.image_align_lambda = image_align_lambda
    ...
    def get_entity_user_embeddings(self):
        """
        Return embeddings with shape (n_users + n_entities, embed_dim).
        - Entity rows combine table embeddings and projected external features using a gate.
        - User rows retain their table embeddings; users have no external features.
        """
        E_table = self.entity_user_embed.weight  # [N_total, D]
        if self.init_entity_features is None:
            return E_table

        # External features are available only for entity rows.
        E_feat = self.init_entity_features
        if self.feat_proj is not None:
            E_feat = self.feat_proj(E_feat)  # [n_entities, D]

        alpha = torch.clamp(self.alpha, 0.0, 1.0)

        # Fuse the entity embeddings.
        E0_entities = alpha * E_table[: self.n_entities] + (1.0 - alpha) * E_feat
        # Keep the user embeddings unchanged.
        E0_users = E_table[self.n_entities :]

        return torch.cat([E0_entities, E0_users], dim=0)

    def image_alignment_loss(self, kg_h, kg_r, kg_t):
        if (self.has_image_rel_id is None) or (self.image_align_lambda <= 0.0):
            return torch.zeros([], device=self.entity_user_embed.weight.device)

        mask = kg_r == self.has_image_rel_id
        if not torch.any(mask):
            return torch.zeros([], device=self.entity_user_embed.weight.device)

        E0 = self.get_entity_user_embeddings()
        e_item = E0[kg_h[mask]]  # Assume each hasImage head is an item.
        e_img = E0[kg_t[mask]]  # The tail is the corresponding image entity.
        return F.mse_loss(e_item, e_img)

    def calc_cf_embeddings(self):
        ego_embed = self.get_entity_user_embeddings()  # (n_users + n_entities, embed_dim)
        all_embed = [ego_embed]
    ...
    def calc_kg_loss(self, h, r, pos_t, neg_t):
        ...
        E0 = self.get_entity_user_embeddings()
        h_embed = E0[h]
        pos_t_embed = E0[pos_t]
        neg_t_embed = E0[neg_t]
        ...
        # Minimize MSE between item embeddings and their image embeddings.
        align_loss = self.image_alignment_loss(h, r, pos_t)
        loss = kg_loss + self.kg_l2loss_lambda * l2_loss + self.image_align_lambda * align_loss
        return loss
